...
#
# End-to-end UEFI Secure Boot ESL/AUTH generation:
# - Use existing PK/KEK/db keys from a PKCS#11 token (e.g. netHSM)
# - Generate CSRs and issue certificates (PK self-signed, KEK by PK, db by KEK)
# - Create ESLs and AUTH blobs (AUTH signed via OpenSSL+ENGINE to avoid
#   efitools ENGINE segfaults)
#
# Requirements:
# - pkcs11-tool (for slot detection by token label)
# - openssl + libengine-pkcs11-openssl (or equivalent pkcs11 engine/provider)
# - efitools: cert-to-efi-sig-list, sign-efi-sig-list
# - uuidgen
#
# You MUST have the PK/KEK/db keys already created on the token/HSM.
# This script will NOT create keys or check that they exist beyond
# whatever failures OpenSSL/engine produces.
# ==========================================================
...
# labels/ids (used only to build PKCS#11 URIs; keys must already exist)
PK_LABEL="tempPKkey";  PK_ID="01";  PK_CN="Platform Key";      PK_DAYS="3650"
KEK_LABEL="tempKEKkey"; KEK_ID="02"; KEK_CN="Key Exchange Key"; KEK_DAYS="3650"
DB_LABEL="tempDBkey";  DB_ID="03";  DB_CN="Database Key";      DB_DAYS="3650"
...
Options:
  --module PATH         PKCS#11 module (.so) [default: $MODULE]
  --token-label LABEL   PKCS#11 token label [default: $TOKEN_LABEL]
  --slot HEX            Slot id (e.g. 0x1234abcd); autodetect by token label if omitted
  --pin PIN             User PIN
  --out DIR             Output directory for CSR/CRT/ESL/AUTH [default: $OUT]
  --engine-conf PATH    OpenSSL engine config (sets OPENSSL_CONF)
  --rsa-pss             Use RSA-PSS for issuer signatures
...
This will:
  - Use existing PK/KEK/db keys on the PKCS#11 token
  - Create CSRs & certs (PK self-signed; KEK by PK; db by KEK)
  - Produce ESL + AUTH files for PK, KEK, db in \$OUT

Prereqs: pkcs11-tool, openssl, efitools (cert-to-efi-sig-list, sign-efi-sig-list), uuidgen.
OpenSSL engine config should point MODULE_PATH to your PKCS#11 module
(or pass --engine-conf).
EOF
      exit 0
      ;;
...
if [[ -z "$SLOT" ]]; then
  log "Detecting slot for token label '$TOKEN_LABEL' ..."
  SLOT_HEX=$(
...
PK_URI="pkcs11:token=${TOKEN_LABEL};object=${PK_LABEL};type=private"
KEK_URI="pkcs11:token=${TOKEN_LABEL};object=${KEK_LABEL};type=private"
DB_URI="pkcs11:token=${TOKEN_LABEL};object=${DB_LABEL};type=private"
...
build_var(){
  ...
  if [[ "$VAR" == "PK" ]]; then
    AUTH_SIGN_CRT="$crt"
    AUTH_SIGN_URI="$URI_SELF"
  elif [[ "$VAR" == "KEK" ]]; then
    AUTH_SIGN_CRT="$OUT/pk.crt"
    AUTH_SIGN_URI="$PK_URI"
  else
    AUTH_SIGN_CRT="$OUT/kek.crt"
    AUTH_SIGN_URI="$KEK_URI"
  fi
